...
$(document).ready(function () {
    ...
    if(queryParams.split('').length > 12){
        ...
        productsArray = productsString[1];
        products = productsArray.split(",")
        console.log(products);
        var productIdArray = queryParamsArray[1];
        productId = productIdArray[productIdArray.length - 1]
    }else{
    ...
    $.ajax({
        ...
        success: function (productData) {
            ...
            productName = $('<div>').text(productData.name);

            productBrand = $('<div>').text(productData.brand);

            productPrice = $('<div>').text("Price: Rs ");

            productPrice.append($('<p>').text(productData.price));

            productDescription = $('<h2>').text("Description");
            productDescription.append($('<p>').text(productData.description));

            productPreview = $('<div>').text("Product Preview");
            productPreviewPhotos = $('<div>').attr('id', 'productPreviewPhotos');
            ...
            addToCart.on("click", function (e) {
                ...
                productQuery = products.toString()
